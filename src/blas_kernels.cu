...
__global__ void sc_copy_kernel(float* X, float* Y, int batch, int c, int wxh, float* scale, float* bias)
{
  int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
  int c_index = (i / wxh) % c;
  if (i < batch * c * wxh)
    Y[i] = X[i] * scale[c_index] + bias[c_index];
}

__global__ void copy_kernel_scale(int N,  float *X, int OFFX, int INCX, float *Y, int OFFY, int INCY, float scale)
{
    int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(i < N) Y[i*INCY + OFFY] = X[i*INCX + OFFX];
}
...
extern "C" void sc_copy_gpu(float* X, float* Y, int batch, int c, int wxh, float* scale, float* bias)
{
  sc_copy_kernel<<<cuda_gridsize(batch*c*wxh), BLOCK>>>(X, Y, batch, c, wxh, scale, bias);
  check_error(cudaPeekAtLastError());
}
...
__global__ void shortcut_kernel(int size, int minw, int minh, int minc, int stride, int sample, int batch, int w1, int h1, int c1, float *add, int w2, int h2, int c2, float* b2, float* s2, float *out)
{
    ...
    out[out_index] = out[out_index] + s2[k]*add[add_index];
    //out[out_index] += add[add_index];
}

extern "C" void shortcut_gpu(int batch, int w1, int h1, int c1, float *add, int w2, int h2, int c2, float* b2, float* s2, float *out)
{
    ...
    shortcut_kernel<<<cuda_gridsize(size), BLOCK>>>(size, minw, minh, minc, stride, sample, batch, w1, h1, c1, add, w2, h2, c2, b2, s2, out);
    check_error(cudaPeekAtLastError());
}
