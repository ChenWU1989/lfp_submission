...
#ifdef GPU
    if (gpu_index >= 0) {
      cuda_push_array(l.crop_scale_gpu, l.crop_scale, c);
      cuda_push_array(l.crop_trans_gpu, l.crop_trans, c);
    }
#endif
    return l;
}
...
layer parse_shortcut(list *options, size_params params, network *net)
{
    ...
    for (int i = 0; i < from.out_c; i++) {
      s.norm_sc_coeff1[i] = s.beta;
    }
    for (int i = 0; i < params.c; i++) {
      s.norm_sc_bias[i] = 0;
      s.norm_sc_coeff0[i] = s.alpha;
    }

#ifdef GPU
    if (gpu_index >= 0) {
      cuda_push_array(s.norm_sc_bias_gpu, s.norm_sc_bias, params.c);
      cuda_push_array(s.norm_sc_coeff1_gpu, s.norm_sc_coeff1, from.out_c);
      cuda_push_array(s.norm_sc_coeff0_gpu, s.norm_sc_coeff0, params.c);
    }
#endif
...
network *parse_network_cfg(char *filename)
{
    ...
    net->norm_in_mean = calloc(net->c, sizeof(float));
    net->norm_in_var = calloc(net->c, sizeof(float));
    net->norm_out_mean = calloc(net->layers[net->n - 1].out_c, sizeof(float));
    net->norm_out_var = calloc(net->layers[net->n - 1].out_c, sizeof(float));
#ifdef GPU
    net->output_gpu = out.output_gpu;
    net->input_gpu = cuda_make_array(net->input, net->inputs*net->batch);
    net->truth_gpu = cuda_make_array(net->truth, net->truths*net->batch);
    net->norm_in_mean_gpu = cuda_make_array(net->norm_in_mean, net->c);
    net->norm_in_var_gpu = cuda_make_array(net->norm_in_var, net->c);
    net->norm_out_mean_gpu = cuda_make_array(net->norm_out_mean, net->layers[net->n - 1].out_c);
    net->norm_out_var_gpu = cuda_make_array(net->norm_out_var, net->layers[net->n - 1].out_c);
#endif
    if(workspace_size){
        //printf("%ld\n", workspace_size);
#ifdef GPU
