...
__device__ float relu_norm_activate_kernel(float x, float norm_mean){return (x>norm_mean) ? x : norm_mean;}
__device__ float elu_activate_kernel(float x){return (x >= 0)*x + (x < 0)*(expf(x)-1);}
__device__ float elu_norm_activate_kernel(float x, float norm_mean){return (x >= norm_mean)*x + (x < norm_mean)*(expf(x)-1);}
__device__ float selu_activate_kernel(float x){return (x >= 0)*1.0507f*x + (x < 0)*1.0507f*1.6732f*(expf(x)-1);}
__device__ float selu_norm_activate_kernel(float x, float norm_mean){return (x >= norm_mean)*1.0507f*x + (x < norm_mean)*1.0507f*1.6732f*(expf(x)-1);}
__device__ float relie_activate_kernel(float x){return (x>0) ? x : .01f*x;}
__device__ float relie_norm_activate_kernel(float x, float norm_mean){return (x>norm_mean) ? x : .01f*x;}
__device__ float ramp_activate_kernel(float x){return x*(x>0)+.1f*x;}
__device__ float ramp_norm_activate_kernel(float x, float norm_mean){return x*(x>norm_mean)+.1f*x;}
__device__ float leaky_activate_kernel(float x){return (x>0) ? x : .1f*x;}
__device__ float leaky_norm_activate_kernel(float x, float norm_mean){return (x>norm_mean) ? x : .1f*x;}
...
__device__ float norm_activate_kernel(float x, ACTIVATION a, float norm_mean)
{
    switch(a){
        case LINEAR:
            return linear_activate_kernel(x);
        ...
        case LOGGY:
            return loggy_activate_kernel(x);
        case RELU:
            return relu_norm_activate_kernel(x, norm_mean);
        case ELU:
            return elu_norm_activate_kernel(x, norm_mean);
        case SELU:
            return selu_norm_activate_kernel(x, norm_mean);
        case RELIE:
            return relie_norm_activate_kernel(x, norm_mean);
        case RAMP:
            return ramp_norm_activate_kernel(x, norm_mean);
        case LEAKY:
            return leaky_norm_activate_kernel(x, norm_mean);
        case TANH:
            return tanh_activate_kernel(x);
        case PLSE:
            return plse_activate_kernel(x);
...
__global__ void norm_activate_array_kernel(float *x, int n, int fmsz, ACTIVATION a, float *norm_mean)
{
    int i = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
    if(i < n) x[i] = norm_activate_kernel(x[i], a, norm_mean[i / fmsz]);
}
...
extern "C" void norm_activate_array_gpu(float *x, int n, int fmsz, ACTIVATION a, float *norm_mean) 
{
    norm_activate_array_kernel<<<cuda_gridsize(n), BLOCK>>>(x, n, fmsz, a, norm_mean);
    check_error(cudaPeekAtLastError());
}
