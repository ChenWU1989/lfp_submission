...
network *make_network(int n)
{
    ...
    net->quantize = calloc(1, sizeof(QUANTIZE));
...
void forward_network(network *netp)
{
    ...
    for(i = 0; i < net.n; ++i){
        ...
        */
        ...
        if ( (l.lfp->type != FP32) && (l.type == SOFTMAX) )
          lfp_de_data(net.input, l.inputs, l.lfp->iexp);
        
        if ((l.type == SOFTMAX) && (net.norm_out_var != NULL) && (net.norm_out_var[0] != 0) )
          denorm_fm(net.input, l.inputs, 1, net.norm_out_mean, net.norm_out_var); 
...
void free_network(network *net)
{
    ...
    if (net->norm_in_mean) {
      free(net->norm_in_mean);
      net->norm_in_mean = NULL;
    }
    
    if (net->norm_in_var) {
      free(net->norm_in_var);
      net->norm_in_var = NULL;
    }

    if (net->norm_out_mean) {
      free(net->norm_out_mean);
      net->norm_out_mean = NULL;
    }

    if (net->norm_out_var) {
      free(net->norm_out_var);
      net->norm_out_var = NULL;
    }
#ifdef GPU
...
void forward_network_gpu(network *netp)
{
    ...
    for(i = 0; i < net.n; ++i){
        ...
        */
        ...
        if ( (l.lfp->type != FP32) && (l.type == SOFTMAX) ) {
          lfp_de_data_gpu(net.input_gpu, l.inputs, l.lfp->iexp);
        }
        
        if ( (l.type == SOFTMAX) && (net.norm_out_var) && (net.norm_out_var[0]) ) {
          denorm_fm_gpu(net.input_gpu, l.inputs, 1, net.norm_out_mean_gpu, net.norm_out_var_gpu);
        }
