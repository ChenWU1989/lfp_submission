...
{  
    ...
    for(i = 0; i < l.batch; ++i){
        for(j = 0; j < l.groups; ++j){
            ...
            if (l.size == 1){
                ...
                if (!net.zero_mean && l.norm_in_var && l.norm_in_var[0]) {
                  float *norm_mean = cuda_make_array(l.norm_in_mean, l.c);
                  float *norm_var = cuda_make_array(l.norm_in_var, l.c);
                  im2col_norm_gpu(im, l.c/l.groups, l.h, l.w, l.size, l.stride, l.pad, b, norm_mean, norm_var, l.lfp->iexp);
                  cuda_free(norm_mean);
                  cuda_free(norm_var);
                }
                else
    ...
    if (!net.zero_mean && l.norm_out_var && l.norm_out_var[0]) {
      ...
      if (l.batch_normalize)
        lfp_de_data_gpu(l.output_gpu, l.outputs, l.lfp->bn_bexp);
      else
