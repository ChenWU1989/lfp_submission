...
__global__ void levels_image_kernel(float *image, float *rand, int batch, int w, int h, int train, float saturation, float exposure, float* translate, float* scale, float shift, int channel)
{
    int size = batch * w * h;
    ...
    image[x + w*(y + h*0)] = rgb.x*scale[c] + translate[c] + (rshift - .5f)*shift;
    image[x + w*(y + h*1)] = rgb.y*scale[c] + translate[c] + (gshift - .5f)*shift;
    image[x + w*(y + h*2)] = rgb.z*scale[c] + translate[c] + (bshift - .5f)*shift;
}
...
extern "C" void crop_gpu(crop_layer layer, network net)
{
    ...
    levels_image_kernel<<<cuda_gridsize(size), BLOCK>>>(net.input_gpu, layer.rand_gpu, layer.batch, layer.w, layer.h, net.train, layer.saturation, layer.exposure, layer.crop_trans_gpu, layer.crop_scale_gpu, layer.shift, layer.out_c);
...
__global__ void crop_kernel(int n, float* x, int w, int h, int c, int out_w, int out_h, float* scale, float* trans, float* y)
{
  int idx = (blockIdx.x + blockIdx.y*gridDim.x) * blockDim.x + threadIdx.x;
  
  int dw = (w - out_w) / 2;
  int dh = (h - out_h) / 2;
  int col = idx % out_w + dw;
  int row = (idx / out_w) % out_h + dh;
  int ch  = (idx / out_w / out_h) % c;

  if (idx < n)
    y[idx] = x[col + row * w + ch * w * h] * scale[ch] + trans[ch];
  
}

extern "C" void center_crop_gpu(crop_layer layer, network net)
{
  int n = layer.out_c * layer.out_h * layer.out_w;
  crop_kernel<<<cuda_gridsize(n), BLOCK>>>(n, net.input_gpu, layer.w, layer.h, layer.c, layer.out_w, layer.out_h, layer.crop_scale_gpu, layer.crop_trans_gpu, layer.output_gpu);
  check_error(cudaPeekAtLastError());
}
